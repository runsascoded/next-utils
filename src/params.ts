...
export function updateHashParams<Params extends { [k: string]: Param<any, any> }>(
    ...
    if (window.location.hash == hashStr) {
        if (log) console.log("updateHashParams: skipping push", url, window.location.hash, window.location.hash == hashStr)
    } else if (push) {
        if (log) console.log("updateHashParams: push (pushState)", url, window.location.hash, window.location.hash == hashStr)
        window.history.pushState({ url: hashStr }, '', hashStr)
    } else {
        if (log) console.log("updateHashParams: push (replaceState)", url, window.location.hash, window.location.hash == hashStr)
        window.history.replaceState({ url: hashStr }, '', hashStr)
    }
    return hash
}

export function updateHashParamsRouter<Params extends { [k: string]: Param<any, any> }>(
    params: Params,
    newVals: { [k: string]: any },
    router: NextRouter,
    { push, log }: { push?: boolean, log?: boolean },
    ...
    const url = { hash }
    if (window.location.hash == hashStr) {
        if (log) console.log("updateHashParamsRouter: skipping push", url, window.location.hash, window.location.hash == hashStr)
        return Promise.resolve(false)
    } else if (push) {
        if (log) console.log("updateHashParamsRouter: push (pushState)", url, window.location.hash, window.location.hash == hashStr)
        return router.push(url, url, { shallow: true })
    } else {
        if (log) console.log("updateHashParamsRouter: push (replaceState)", url, window.location.hash, window.location.hash == hashStr)
        return router.replace(url, url, { shallow: true })
    }
}
export function parseHashParams<Params extends { [k: string]: Param<any, any> }, ParsedParams>(
    ...
    useEffect(
        () => {
            ...
            // console.log("replaceState:", stateHash)
            // window.history.replaceState({ hash: stateHash }, '', `#${stateHash}`)
